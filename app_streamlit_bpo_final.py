...
st.sidebar.image("images/trayectoria.png", use_column_width=True)
st.sidebar.title("📂 Procesador de Archivos BPO")
st.sidebar.markdown("Sube tu archivo Excel y descarga uno limpio con fechas corregidas y agentes BPO asignados automáticamente.")
...
if uploaded_file:
    ...
    nombre_archivo = f"Archivo_Procesado_BPO_{datetime.today().strftime('%Y%m%d')}.xlsx"
    df_final.to_excel(nombre_archivo, index=False)

    with open(nombre_archivo, "rb") as f:
        b64 = base64.b64encode(f.read()).decode()

    href = f'<a href="data:application/octet-stream;base64,{b64}" download="{nombre_archivo}">📥 Descargar archivo procesado</a>'
